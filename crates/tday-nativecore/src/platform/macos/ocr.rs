...
pub fn find_text_ocr(
    ...
    // display_id == 0 or None → main display; otherwise match by CGDirectDisplayID.
    let disp = displays
        .iter()
        ...
        .or_else(|| displays.first())    // fallback: first available display
        .cloned()
        .ok_or("No display found")?;

    // Capture the display by CGDirectDisplayID.
    // `screencapture -D <id>` accepts both 1-based index AND CGDirectDisplayID
    // on macOS 10.15+.  Passing the hardware ID directly is unambiguous and
    // works correctly with external monitors regardless of enumeration order.
    ...
    let status = Command::new("/usr/sbin/screencapture")
        .args(["-x", "-D", &disp.id.to_string(), path_str])
        .status()
