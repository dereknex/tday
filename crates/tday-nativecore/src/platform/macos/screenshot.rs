...
///
/// Strategy:
///   1. Try `CGDisplayCreateImage` directly (fast, no subprocess, works in most Electron / app contexts).
///   2. Fall back to `screencapture` CLI (needs Screen Recording TCC permission in the
///      parent process — Terminal.app, VS Code, etc.).
...
pub fn capture_screen() -> Result<Screenshot, String> {
    ...
    // --- Strategy 1: CGDisplayCreateImage (no subprocess, works from Electron/app) ---
    if let Ok(png_data) = capture_screen_cg_direct() {
        let (pw, ph) = png_dims(&png_data);
        return Ok(Screenshot { png_data, scale_factor: scale, origin_x: ox, origin_y: oy, pixel_width: pw, pixel_height: ph });
    }

    // --- Strategy 2: screencapture CLI fallback ---
...
/// Capture the main display directly via CGDisplayCreateImage (no subprocess).
/// Returns the screenshot as raw PNG bytes.
fn capture_screen_cg_direct() -> Result<Vec<u8>, String> {
    ...
    // CGWindowListCreateImage with kCGNullWindowID captures the whole screen.
    // Returns None if Screen Recording permission has not been granted.
    ...
    .ok_or_else(|| "CGWindowListCreateImage returned null (no Screen Recording permission?)".to_string())?;

    cg_image_to_png(&cg_image)
}

/// Convert a `CGImage` to PNG bytes (BGRA → RGB → PNG encode).
