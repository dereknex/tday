...
pub fn get_displays() -> Result<Vec<DisplayInfo>, String> {
    ...
    // Prefer CGGetOnlineDisplayList — it returns displays even when they are not
    // "active" (e.g. remote desktop, clamshell, or mirrored setups where
    // CGGetActiveDisplayList incorrectly returns an empty list).
    #[link(name = "CoreGraphics", kind = "framework")]
    extern "C" {
        fn CGGetOnlineDisplayList(max: u32, out: *mut u32, count: *mut u32) -> i32;
    }
    let ret = unsafe { CGGetOnlineDisplayList(16, ids.as_mut_ptr(), &mut count) };
    if ret != 0 {
        return Err(format!("CGGetOnlineDisplayList failed: {ret}"));
    }

    // If online list is also empty fall back to active list (shouldn't happen)
    if count == 0 {
        let ret2 = unsafe { core_graphics::display::CGGetActiveDisplayList(16, ids.as_mut_ptr(), &mut count) };
        if ret2 != 0 {
            return Err(format!("CGGetActiveDisplayList fallback failed: {ret2}"));
        }
    }
