...
| # | Feature | Description |
|---|---|---|
| 1 | **Computer Use** (macOS) | Give any agent hands on your Mac — **no vision model required**. Uses macOS Accessibility APIs to read and control any native app directly, without screenshots. One-click enable for **Claude Code, Codex, Gemini, OpenCode, and Pi**. Agents can click UI elements, type, scroll, open apps, control Chrome via CDP, and fall back to OCR when needed. Built-in **Computer Operator** CoWorker + one-click Accessibility & Screen Recording permission setup. |
